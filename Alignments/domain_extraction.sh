...
## $3 = symfrac value
## $4 = OG-vs-proteome order of magnitude difference (for evalue). Ex: 100 sequences in the OG, number of sequences more or less two orders of magnitude lower than the average proteome (~10^4 sequences) ==> evalue cutoff for hmmsearch is two exponents lower
## Fasta files with OG sequences need .fasta extension
...
for i in $(grep -f "$2" "$1" | awk -F "\t" '$9<=1e-3' | awk '{print $1}' | sort -u ); 
	do species=$(echo "$i" | grep -oh -f /home/STUDENTI/shared/dataset_2022.txt)
	for m in $(grep "$i" /home/STUDENTI/shared/Interproscan/"$species"/"$species"*.fasta.gff3 | grep -f "$2" | awk -F "\t" '$6<=1e-3' | perl -lne '/(?<=ID\=)(.+?)(?=\;)/ and print $1')
